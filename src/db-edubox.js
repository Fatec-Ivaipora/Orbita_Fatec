...
// as outras rotas, então um throw aqui derrubaria o app inteiro (todas as
// rotas, não só a CPA) sempre que EDUBOX_* não estiver configurado (ex.:
// esquecer de configurar as env vars na Vercel). Em vez disso, `pool` fica
// `null` e o erro só acontece dentro de `query()`, isolado à rota que
// efetivamente precisar do Edubox.
let pool = null;
...
if (process.env.EDUBOX_HOST) {
    ...
    console.error('⚠️  EDUBOX_HOST não configurado — o módulo CPA vai retornar erro até EDUBOX_HOST/EDUBOX_DATABASE/EDUBOX_USER/EDUBOX_PASSWORD serem definidos (.env local ou variáveis de ambiente da Vercel).');
}
